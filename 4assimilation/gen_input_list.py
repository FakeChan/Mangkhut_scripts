...
link_target='/share/home/lililei1/kcfu/tc_mangkhut/4assimilation/0mem_all_time/10_00_00_noinflatedOcean'
...
with open(f"{link_target}/input_list_{domain}.txt", "w", encoding="utf-8") as file:
    for imem,mem in enumerate(memlist):
        ...
        subprocess.run(['mkdir','-p',f'{link_target}'])
        subprocess.run(['cp',f'{ensmem}/wrfout_{domain}_2018-09-{day}_{hour}:{minute}:00',f'{link_target}/firstguess_{domain}.mem{imem+1:03d}'])
