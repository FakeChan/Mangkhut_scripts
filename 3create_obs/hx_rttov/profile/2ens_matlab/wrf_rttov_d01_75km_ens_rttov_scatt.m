...
delta_x=1500;%delta_x is the resolution (in m) of ensemble 
...
% % iloc=ilist(tloc)+1;
% % jloc=jlist(tloc)+1;
% jloc=round(jlist(tloc)/5)+j_parent_start+1
% iloc=round(ilist(tloc)/5)+i_parent_start+1

%fkc.2025.12.3 in case of mismatch of ens domain and NR domain, use function match_typhoon_grid.m
[center_lat,center_lon,iloc,jloc]=match_typhoon_grid(strcat(NR_location,NR_fname),ens_fname)
delta=7500/delta_x;
%=============================================
...
for yloc=jloc-radius*delta:delta:jloc+(radius+1)*delta
    for xloc=iloc-radius*delta:delta:iloc+(radius+1)*delta
            ...
            for kk = nz:-1:1
                ...
                % --- 写入一行 ---
                % (7) Total Ice: Ice + Snow + Graup (kg/kg)
