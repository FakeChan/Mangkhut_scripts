...
# in cylcing DA, ens_wrfout_dir is passed from envs
...
for imem in ${memlist[*]};do
	...
	if [[ ! -d ${ens_wrfout_dir} ]];then
		echo "warning the folder does not exist:$member"
		continue
	fi
	echo "======================================================"
	echo "fkc:ensBT driver: calculating $member at $current_time"
	echo "fkc:the ensemble comes from ${ens_wrfout_dir}"
	echo "======================================================"
	export ens_mem_dir=${ens_wrfout_dir}/
