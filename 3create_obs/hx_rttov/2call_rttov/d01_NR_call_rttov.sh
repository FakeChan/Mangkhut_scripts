...
cd    $call_rttov_dir
mkdir -p ${call_rttov_dir}/call_rttov_test ${prof_dir}/${instrument}/ ${obs_dir}/${instrument}
...
ln -sf ${prof_dir}/prof${obs_day}_${obs_hour}:${obs_min}.dat ${call_rttov_dir}/call_rttov_test/prof${time}.dat
$call_rttov_dir/${running_script_file} ARCH=gfortran

if [ $rttov_scatt -eq 0 ];then
    mv $call_rttov_dir/call_rttov_test/prof${time}.dat.gfortran ${prof_dir}/${instrument}/NR_${domain}_output${time}.dat
    #sed -n '49,99999999p' ${prof_dir}/${instrument}/NR_${domain}_output${time}.dat > ${obs_dir}/${instrument}/NR_${domain}_output_${time}.txt
    export BT_output_dir=${prof_dir}/${instrument}
    export BT_input=NR_${domain}_output${time}.dat
    export BT_output=NR_${domain}_output_${time}.txt
    export BT_write_dir=${obs_dir}
    python ${work_dir}/2call_rttov/extractBT_rttov_scatt.py
else
    export BT_output_dir=${prof_dir}/${instrument}
    export BT_input=NR_${domain}_output${time}.dat
    export BT_output=NR_${domain}_output_${time}.txt
